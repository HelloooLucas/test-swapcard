...
const AlbumDetails:React.FC<AlbumDetailsProps> = ({ match, history }) => {
    const albumId = match.params.id;
    const { loading, error, data } = useQuery(FIND_ALBUM, { variables: { id: albumId }})
    const album = data?.node;
    const imgUrl = album?.coverArtArchive.front || noImage;

    return (
        <Wrapper>
            <BackButton onClick={history.goBack}>{'< Back to artist'}</BackButton>
